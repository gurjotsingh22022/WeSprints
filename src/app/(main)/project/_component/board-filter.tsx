...
export default function BoardFilters({ issues, onFilterChange, filterNeeded=false }: Props) {
  ...
  useEffect(() => {
    const filteredIssues = issues.filter(
      (issue: any) =>
        ...
        (selectedAssignees.length === 0 || selectedAssignees.includes(issue?.assigneeId)) &&
        (selectedPriority === "" || issue.priority === selectedPriority)
    );
    onFilterChange(filteredIssues);
  }, [searchTerm, selectedAssignees, selectedPriority, issues, filterNeeded]);
  ...
  return (
    <>
    {
        filterNeeded?
        <>
        <div className="space-y-4 mb-4 animate-filter-tab" suppressHydrationWarning>
